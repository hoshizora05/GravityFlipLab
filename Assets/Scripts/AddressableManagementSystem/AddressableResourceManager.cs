...
namespace AddressableManagementSystem
{
    ...
    public class AddressableResourceManager : MonoBehaviour
    {
        ...
        public async Task<bool> Initialize()
        {
            ...
            // ★ autoReleaseHandle=false でハンドルを保持する
            var initOperation = Addressables.InitializeAsync(false);
            try
            {

                // ここで IResourceLocator が返ってくる
                var locator = await initOperation.Task;

                // 追加のエラーチェックは locator == null で十分
                _isInitialized = true;
                Debug.Log("Addressables 初期化成功");
            ...
            {
                Debug.LogError($"Addressables 初期化失敗: {e.Message}");
